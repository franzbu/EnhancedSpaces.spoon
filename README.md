...
Every power user sooner or later feels restricted by the confinements of a single screen; there is simply no such thing as too much space. While a multi-monitor arrangement provides the obvious remedy, there is a lesser-known solution that enables the user to have more than one screen on a single display: virtual screens or, as they are called in EnhancedSpaces, mSpaces.
...
So I decided to develop an application that manages virtual screens and windows in the way I've always wanted. One of the main goals has been to make managing Spaces and windows straightforward, intuitive, and efficient.
...
Configuring EnhancedSpaces involves rolling up your sleeves and grabbing your keyboard. While it is understandable that even some power users prefer a graphical user interface for adjusting the preferences of their applications, there are substantial advantages to text based configuration that are easy to miss unless experienced first-hand, one of them being that once you've got the knack of it, making changes to your setup is straightforward and efficient, which are reasons you have become interested in EnhancedSpaces in the first place, am I right?

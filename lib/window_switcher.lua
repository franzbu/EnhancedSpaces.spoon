...
local function exit(self)
  ...
  -- windows[selected]:unminimize() -- original: not commented out
  --  if windows[selected]:application():bundleID()~='com.apple.finder' then
  --    windows[selected]:focus()
  --  else
  --windows[selected]:focus() -- original: not commented out
  --  end
end
...
local function show(self,dir,windowsOnCurrentMSpace) -- original: without 'windowsOnCurrentMSpace'
  ...
  if not selected then -- fresh invocation, prep everything
    ...
    self.drawDelayed=timer.doAfter(0.001,function() -- original: 0.15
